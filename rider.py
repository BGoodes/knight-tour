...
class Rider(pygame.sprite.Sprite):
    ...
    def move_to_position(self, x: int, y: int):
        ...
        if int(abs(pos.x - x) + abs(pos.y - y)) != 3:
            error("Erreur : nombre de case parcouru incorrect")
            return
...
def error(log):
    ...
    Tk().wm_withdraw()
    messagebox.showinfo("Erreur /!\\", log)

    pygame.quit()


def display_result():
    pygame.display.set_caption("Cavaliers -> Nombre de case validées : " + str(len(check_cells)))

    Tk().wm_withdraw()
    messagebox.showinfo("Résultats", "Nombre de case validées : " + str(len(check_cells)))


if __name__ == '__main__':
    pygame.init()

    pygame.display.set_caption("Cavaliers")
